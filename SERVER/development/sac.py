...
class Actor(nn.Module):
    ...
    def forward(self, state, deterministic=False, with_logprob=True):
        ...
        if deterministic:
            ...
            # reparameterization trick of Gaussian
            u = dist.rsample()  
        a = torch.tanh(u)
...
class SAC_Agent(object):
    def __init__(
        ...
        if adaptive_alpha:
            ...
            self.target_entropy = torch.tensor(
                -action_dim, dtype=float, requires_grad=True, device=device
            )
            # We learn log_alpha instead of alpha to ensure exp(log_alpha)=alpha>0
            self.log_alpha = torch.tensor(
                np.log(alpha), dtype=float, requires_grad=True, device=device
            )
    ...
    def save(self, episode):
        ...
        torch.save(
            self.q_critic.state_dict(), "./model/sac_q_critic{}.pth".format(episode)
        )

    def load(self, episode):
        self.actor.load_state_dict(
            torch.load("./model/sac_actor{}.pth".format(episode))
        )
        self.q_critic.load_state_dict(
            torch.load("./model/sac_q_critic{}.pth".format(episode))
        )
